...
#define TELEMETRY_PROP_NAME_Power "power"
...
/* --- Internal Functions --- */
static float simulated_get_temperature() { return 50.0; }

static float simulated_get_humidity() { return 160.0; }

static float simulated_get_ambientLight() { return 2.5; }

static void simulated_get_power_altitude(float* power, float* altitude)
{
    ...
    size_t* payload_buffer_length)
{
  ...
  az_span json_span;
  float temperature, humidity, light, power, altitude;
  ...
  simulated_get_power_altitude(&power, &altitude);
  ...
  rc = az_json_writer_append_double(&jw, power, DOUBLE_DECIMAL_PLACE_DIGITS);
  EXIT_IF_AZ_FAILED(
